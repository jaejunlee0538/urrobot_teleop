...
namespace ur_kinematics {
  ...
  double* findBestSolution(double* solutions,const double * q,const double* w)
  {
    static const double ADD_ANGLE[3] = {-2.0*M_PI, 0.0, 2.0*M_PI};
    double * sols[8] =
    {NULL,NULL,NULL,NULL,
        NULL,NULL,NULL,NULL};

    //validate every solutions.
    for(int i=0;i<8;i++)
    {
      sols[i] = solutions;
      solutions += 6;
      double test_solution[6] =
      {INVALID_ANGLE,INVALID_ANGLE,INVALID_ANGLE,
          INVALID_ANGLE,INVALID_ANGLE,INVALID_ANGLE};
      ...
      for( ;k<6;k++)
      {
        for(int m=0;m<3;m++)
        {
          double test_angle = sols[i][k] + ADD_ANGLE[m];
          if(fabs(test_angle)<=2.0*M_PI && fabs(test_angle - q[k])<fabs(test_solution[k] - q[k]))
            test_solution[k] = test_angle;
        }
        if(test_solution[k] == INVALID_ANGLE)
          break;
      }
      ...
      if(k!=6){
        ...
        // solution is valid
        ::memcpy(sols[i],test_solution,sizeof(double)*6);
      }
    }

    //Find best solution
    double* best_solution = NULL;
    double best_metric = 99999999.0;
    for(int i=0;i<8;i++)
    {
      if(sols[i]==NULL)
        continue;
      ...
      for(int k=0;k<6;k++)
      {
        temp = temp + pow(w[k]*(sols[i][k]-q[k]),2);
      }
      if(temp < best_metric){
  ...
  int testInverse(double * q)
  {
    ...
    double* best_solution = NULL;
    best_solution = findBestSolution(q_sols,q,weights);
    ...
    if(best_solution == NULL)
      ...
      for(int i=0;i<6;i++)
      {
        if(fabs(q[i] - best_solution[i]) > 0.000001)
          return 0;
      }
      return 1;
    }
